...
def upload_and_save_image(url, file):
    with open(file, 'rb') as file:
        ...
        response.raise_for_status()
        uploading_properties = response.json()
    owner_id, media_id = save_image(uploading_properties['server'],
...
def save_image(server, photo, hash_code,
    ...
    response = requests.post(url.format('photos.saveWallPhoto'),
                             params=params)
    response.raise_for_status()
    saving_properties = response.json()['response'][0]
    return saving_properties['owner_id'], saving_properties['id']
...
def get_comic_properties(comic_id, url=COMIC_API_URL):
    ...
    response = requests.get(url)
    response.raise_for_status()
    comic_properties = response.json()
    return comic_properties
